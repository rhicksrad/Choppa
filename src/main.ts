...
damage.setDamageModifier((entity, amount) => {
  if (entity === player) {
    if (playerState.invulnerable) return 0;
    const mitigation = 1 - Math.min(0.5, playerUpgrades.armor * 0.12);
...
let outOfFuelSeconds = 0;
let lowFuelWarning = 0;
...
const loop = new GameLoop({
  update: (dt) => {
    ...
    const fuel = fuels.get(player);
    if (fuel) {
      const maxFuel = fuel.max || 1;
      if (fuel.current <= 0.01) {
        outOfFuelSeconds += dt;
        lowFuelWarning = Math.min(1.5, lowFuelWarning + dt * 3);
        if (!playerState.invulnerable) {
          const crashDelay = 1.5;
          if (outOfFuelSeconds > crashDelay) {
            const ramp = Math.min(1.5, outOfFuelSeconds - crashDelay);
            const perSecond = 12 + ramp * 18;
            damage.applyDirect(player, perSecond * dt);
          }
        }
        engine.setIntensity(0);
      } else {
        const ratio = fuel.current / maxFuel;
        if (ratio < 0.25) lowFuelWarning = Math.min(1.2, lowFuelWarning + dt * 1.8);
        else lowFuelWarning = Math.max(0, lowFuelWarning - dt * 1.2);
        outOfFuelSeconds = 0;
      }
    }
    updatePickups();
  ...
  render: () => {
    ...
    if (lowFuelWarning > 0 && ui.state === 'in-game' && !playerState.invulnerable) {
      context.save();
      context.globalAlpha = Math.min(1, 0.35 + lowFuelWarning * 0.4);
      context.fillStyle = outOfFuelSeconds > 0 ? '#ef476f' : '#ffd166';
      context.font = 'bold 20px system-ui, sans-serif';
      context.textAlign = 'center';
      const fuelMessage = outOfFuelSeconds > 0 ? 'OUT OF FUEL — RETURN TO PAD' : 'LOW FUEL';
      context.fillText(fuelMessage, w / 2, h * 0.85);
      context.restore();
    }
